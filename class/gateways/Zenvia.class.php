...
class Zenvia {
	 ...
	 * 
	 * Parametros:
	 * @param $msg_id = id da mensagem no banco de dados para saber qual a mensagem que 
	 * 				foi enviada na resposta
	 * @param $msg = mensagem em texto corrido não pode haver quebra de linha
	 * @param $contatos = array com os contatos para onde vai ser enviado no formato 
	 * 				chave = id do contato, valor = celular do contato
	 * @param $agendamento = passa a data e hora para ser disparado o sms
	 */ 
	public function sendSMS ($msg_id=null, $msg=null, $contatos=null, $agendamento=null) {
		...
		if(!is_null($agendamento)) {
			$ag = '"schedule":"'.$this->getDataZenvia($agendamento).'",';
		} else {
			$ag = "";
		}
	...
	/**
	 * Metodo para formatar dentro dos metodos do zenvia
	 * 
	 * 
	 */
	public function getDataZenvia($data) {
		//formata a data
		$data = substr(date(DATE_ISO8601, strtotime($data)),0,-5);
		//retorna a mesma
		return $data;
	}
	...
	/**
	 * Metodo para cancelar sms agendado
	 * 
	 * Parametros
	 * 
	 * @param chave_envio -> id do envio
	 * 
	 */ 
	public function cancelarAgSMS($chave_envio) {

		//chama o curl para conectar no zenvia
		$ch = curl_init();
		...
		//seta os parametros
		curl_setopt($ch, CURLOPT_URL, "https://api-rest.zenvia360.com.br/services/cancel-sms/".$chave_envio);		
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
		curl_setopt($ch, CURLOPT_HEADER, FALSE);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
		//fala que vai utilizar post
		...
		//retorna o detalhe para imprimir na tela o envio
		/*if($resp->cancelSmsResp->statusCode == 09 ) {
			return true;
		}
		//retorna o erro
		return $resp->cancelSmsResp->detailDescription;
		*/
